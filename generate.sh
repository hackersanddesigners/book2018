...
pathlib.Path( src_path / 'clean' ).mkdir( exist_ok=True )
...
def main(argv):
   ...
   outputfile = ''
   try:
      opts, args = getopt.getopt(argv,"hcb",["clean", "build"])
...
def build():
	...
	subprocess.Popen(['weasyprint %s %s' % (dest_file, pdf_path)], shell = True, stdout=subprocess.DEVNULL)
